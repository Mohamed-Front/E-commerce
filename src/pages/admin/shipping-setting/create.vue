...
const shippingData = ref({
  ...
  zones: [
    { from: 0, to: 0, fee: 0, express_time: 0, express_fee: 0 }
  ]
});
...
const fetchStores = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: t('error'),
      detail: t('shippingSetting.loadStoresError'),
      life: 3000
    });
  }
};
...
const submitForm = async () => {
  ...
  try {
    ...
    await axios.post("/api/shipping-setting", shippingData.value, {

    });
    toast.add({
      severity: 'success',
      summary: t("success"),
      detail: t("shippingSetting.createdSuccess"),
      life: 3000
    });
  } catch (error) {
    console.error("Error:", error);
    const errorMessage = error.response?.data?.message || t("shippingSetting.createError");
...
<template>
  <div v-can="'create shipping settings'" class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-lg">
    <h1 class="text-3xl font-bold text-center mb-8 text-gray-800">{{ t('shippingSetting.createTitle') }}</h1>
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
      ...
      <div class="pt-4 flex justify-center space-x-4">
        ...
        <Button
          type="submit"
          :label="t('shippingSetting.createShipping')"
          icon="pi pi-plus"
          :loading="loading"
          ...
          :disabled="loading"
        >
          <span v-if="!loading">{{ t('shippingSetting.createShipping') }}</span>
