...
const toggleGroup = (groupName, event) => {
  ...
  if (event.target.checked) {
    // Add all group permissions if not already selected
...
const isGroupSelected = (groupName) => {
  ...
  return groupItems.every(item =>
    selectedPermissions.value.includes(item.id)
  );
};
...
const isGroupIndeterminate = (groupName) => {
  ...
  const hasSome = groupItems.some(item =>
    selectedPermissions.value.includes(item.id)
  );
  const hasAll = groupItems.every(item =>
    selectedPermissions.value.includes(item.id)
  );
