...
const submitForm = async () => {
  loading.value = true;
couponData.value.expires_at=moment(couponData.value.expires_at).format('YYYY-MM-DD');
  try {
    await axios.post("/api/coupon", couponData.value);
...
<template>
  <div v-can="'create coupons'" class="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg">
    <h1 class="text-3xl font-bold text-center mb-8 text-gray-800">{{ $t("coupon.create_new_coupon") }}</h1>
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
      ...
      <div class="pt-4 flex justify-center">
        <Button
          ...
          :disabled="loading"
        >
          <span v-if="!loading">{{ $t("coupon.create_coupon") }}</span>
