  ...
  <div class="bg-gray-900 min-h-screen flex items-start justify-center p-4 overflow-hidden" dir="rtl">
    <div class="bg-white rounded-3xl shadow-2xl p-4 w-full max-w-sm sm:max-w-lg my-8">
      ...
      <form @submit.prevent="authStore.handleLogin({ type: 'phone', phoneNumber, password })" v-if="selectedTab === 'phone'" class="space-y-6">
        <div class="flex">
          ...
          <input
            ...
            class="w-2/3 px-4 py-3 rounded-tl-xl rounded-bl-xl bg-gray-100 text-gray-700 focus:outline-none transition-shadow text-right"
            required
            pattern="[0-9]{9,12}"
          />
        </div>
        ...
        <div class="relative">
          <input
            ...
            class="w-full px-4 py-3 pr-12 rounded-xl bg-gray-100 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow text-right"
            required
            minlength="8"
          />
          <button
          ...
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                ...
                d="M3 3l18 18M4.48 15.52A9.965 9.965 0 0112 17c4.478 0 8.268-2.943 9.543-7-.29-.93-.73-1.811-1.313-2.617"
              />
              <path
                v-else
                stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="text-left text-sm">
          <a @click="router.push({name:'forgetpassword'})" class="text-blue-600 font-medium hover:underline">نسيت كلمة المرور؟</a>
        </div>
        ...
        <div v-if="authStore.getErrorsByAction('handleLogin').length" class="text-red-600 text-sm text-center">
          <p v-for="error in authStore.getErrorsByAction('handleLogin')" :key="error">{{ error }}</p>
        </div>
      </form>

      <!-- Email Login -->
      <form @submit.prevent="authStore.handleLogin({ type: 'email', email, password })" v-else class="space-y-6">
        <input
          type="email"
          v-model="email"
        ...
        <div class="text-left text-sm">
          <a @click="router.push({name:'forgetpassword'})" class="text-blue-600 cursor-pointer font-medium hover:underline">نسيت كلمة المرور؟</a>
        </div>
      ...
      <button
        ...
        :disabled="authStore.loading"
      >
        <svg xmlns="http://www.w3.org/2000/svg" aria-label="Google" role="img" viewBox="0 0 512 512" width="34px" height="34px" fill="#000000">
      ...
      <div v-if="authStore.getErrorsByAction('googleLogin').length" class="text-red-600 text-sm text-center mt-4">
        <p v-for="error in authStore.getErrorsByAction('googleLogin')" :key="error">{{ error }}</p>
      </div>

      <div class="mt-8 text-center text-sm text-gray-500">
        أليس لديك حساب؟
        <a @click="router.push({name:'register'})" class="text-blue-600 cursor-pointer font-semibold hover:underline">سجل الآن</a>
      </div>
    </div>
  </div>
</template>
...
watch(selectedTab, () => {
  authStore.clearErrors('handleLogin');
  authStore.clearErrors('googleLogin');
});

const handleGoogleLogin = async () => {
  authStore.clearErrors('googleLogin'); // Clear previous Google login errors
  try {
    // Placeholder for Google login implementation
    console.log('Attempting to log in with Google.');
    // Example with Firebase (uncomment and configure if using Firebase):
    /*
    ...
    await authStore.handleGoogleLogin({ token: user.accessToken });
    */
  } catch (error) {
    authStore.authErrors['googleLogin'] = [error.message || 'فشل تسجيل الدخول باستخدام جوجل'];
  }
};
</script>
