...
<template>
  ...
  <div class="min-h-screen p-8 font-inter" dir="rtl">
    ...
    <div class="max-w-6xl mx-auto flex flex-col md:flex-row gap-6">
      ...
      <!-- Main Content section with address creation/update form -->
      <main
...
一点
            ...
            <div class="space-y-2">
              <label class="flex items-center space-x-2">
                <input
                  ...
                  :binary="true"
                />
                <span class="text-sm font-medium text-gray-700 mx-1">{{
          ...
          <div class="pt-4 flex justify-center">
            <Button
              ...
              :disabled="loading"
            >
              <span v-if="!loading">{{ isEditing ? $t('address.update_address') : $t('address.create_address') }}</span>
...
// Fetch address data if ID is present
const fetchAddress = async (id) => {
  try {
    ...
    addressData.value = {
      ...
      is_default: data.is_default === '1' || data.is_default === true,
...
const geocodeAddress = async () => {
  try {
    const response = await axios.get('https://maps.googleapis.com/maps/api/geocode/json', {
      params: {
        address: `${addressData.value.address_line_1}, ${addressData.value.city}, ${addressData.value.country}`,
    ...
    if (response.data.results.length > 0) {
      ...
      addressData.value.long = location.lng;
      toast.add({
        severity: 'success',
        summary: t('success'),
        detail: t('address.location_found'),
        life: 3000,
      });
    } else {
      toast.add({
        severity: 'warn',
        summary: t('warning'),
        detail: t('address.no_location_found'),
        life: 3000,
      });
    }
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: t('error'),
      detail: t('address.geocode_failed'),
      life: 3000,
    });
  }
};
...
const submitForm = async () => {
  ...
  try {
    // Validation
    if (
      !addressData.value.address_line_1 ||
    ...
    let message = isEditing.value ? t('address.failed_to_update') : t('address.failed_to_create');
    if (error.response) {
      if (error.response.status === 400) {
        message = t('address.invalid_input');
      } else if (error.response.status === 401) {
        message = t('address.unauthorized');
      }
    }
    toast.add({
      severity: 'error',
      summary: t('error'),
      detail: error.response?.data?.message || message,
      life: 3000,
    });
  } finally {
    loading.value = false;
