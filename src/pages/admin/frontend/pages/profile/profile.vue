    ...
    <div class="max-w-6xl mx-auto flex flex-col md:flex-row gap-6">
      ...
      <aside class="w-full md:w-64 bg-white rounded-xl shadow-md p-6 h-fit">
        <ul class="space-y-2">
          ...
          <li @click="router.push({ name: 'profile' })" class="p-3 rounded-lg flex items-center gap-4 bg-yellow-100 text-yellow-700 font-medium cursor-pointer">
            ...
            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="bg-white border border-gray-200 rounded-xl p-6 flex flex-col items-start gap-4 shadow-sm">
                <div class="flex items-center w-full justify-between">
                  <h3 class="text-lg font-bold text-gray-700">نظرة عامة حول الحساب</h3>
                  <button @click="router.push({ name: 'editprofile' })" class="text-gray-400 hover:text-gray-600 transition-colors">
                ...
                <div class="flex items-center gap-4 w-full">
                  <img src="https://placehold.co/80x80/606c38/FFF" alt="User Profile" class="w-20 h-20 rounded-full object-cover">
                  <div class="flex flex-col items-start">
                    <p class="font-bold text-gray-800 text-xl">{{ username }}</p>
                    <p class="text-gray-500 text-sm ltr:font-inter ltr:direction-ltr">{{ phoneNumber }}</p>
                  </div>
                </div>
                <button @click="router.push({ name: 'editprofile' })" class="w-full mt-4 py-3 px-6 bg-transparent border border-blue-500 text-blue-500 rounded-xl font-medium hover:bg-blue-50 transition-colors duration-200">
                  تعديل بيانات الحساب
                </button>
              </div>
              <!-- Primary Address -->
              <div class="bg-white border border-gray-200 rounded-xl p-6 flex flex-col items-start gap-4 shadow-sm">
                <div class="flex items-center w-full justify-between">
                  <h3 class="text-lg font-bold text-gray-700">العنوان الرئيسي</h3>
                  <button @click="router.push({ name: 'addresses' })" class="text-gray-400 hover:text-gray-600 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                    </svg>
                  </button>
